...
<title> AWS ændringer til OSM </title>
</head>
<body>
...
$osmdatalat = array('Mangler');
$osmdatalon = array('Mangler');
$osmdatafulllat = array('Mangler');
$osmdatafulllon = array('Mangler');
$osmdataident = array('Mangler');
$osmdatarev = array('Mangler');
$osmdatasource = array('Mangler');
$osmdatamunicipalityno = array('Mangler');
$osmdatastreetno  = array('Mangler');
$osmfixme = array('Mangler');
$osmoisfixme = array('Mangler');
...
$fulllat = $val[lat];
$fulllon = $val[lon];
array_push($osmdatafulllat,$fulllat);
array_push($osmdatafulllon,$fulllon);
array_push($osmdatalat,$lat);
array_push($osmdatalon,$lon);
...
array_push($osmfixme,$val[tags]['fixme']);
array_push($osmoisfixme,$val[tags]['ois:fixme']);
array_push($osmdatarev,$osakrevision);
array_push($osmdatasource,$val[tags]['source']);
array_push($osmdatamunicipalityno,$val[tags]['osak:municipality_no']);
array_push($osmdatastreetno,$val[tags]['osak:street_no']);
array_push($osmdataident,$val[tags]['osak:identifier']);
...
if (empty($osmfixme[$soeg]) && empty($osmoisfixme[$soeg]) ) 
{

if (!$soeg2)
{
	if (!($lat == $osmdatalat[$soeg]) && !($lon == $osmdatalon[$soeg]) && !($lon == "0.00") && !($lat == "0.00"))
{
	
echo "Henter noden ". $osmdataid[$soeg] . "<BR>";
$getnode = file_get_contents('http://www.openstreetmap.org/api/0.6/node/'. $osmdataid[$soeg]);

$nodestart = strpos($getnode, '<node');
$getnode = substr_replace($getnode, '', 1, (int)$nodestart);
$getnode = str_replace($osmdatafulllat[$soeg],$lat,$getnode);
$getnode = str_replace($osmdatafulllon[$soeg],$lon,$getnode);
$getnode = str_replace($osmdatarev[$soeg],$osakrevision,$getnode);
$getnode = str_replace($osmdatasource[$soeg],'Geodatastyrelsens Adresse Web Services (AWS)',$getnode);
$getnode = str_replace($osmdatamunicipalityno[$soeg],$osakmunicipalityno,$getnode);
$getnode = str_replace($osmdatastreetno[$soeg],$osakstreetno,$getnode);
$getnode = str_replace($osmdataident[$soeg],$awsid,$getnode);
$getnode = str_replace("node id","node action='modify' id",$getnode);
$getnode = str_replace("</osm>","",$getnode);
...
echo $osmdataid[$soeg] . ";" . $fulladdr . ";" ;
echo $osmdatalat[$soeg] . " --> " . $lat . ". " . $osmdatalon[$soeg] . " --> " . $lon . ". ";
echo $osmdatarev[$soeg] . " --> " . $osakrevision . ". ";
echo $osmdataident[$soeg] . " --> " . $awsid . ". ";
...
//insert to SQL
$sqltextexport = iconv("UTF-8","ISO-8859-1",str_replace("'","#","$textexport2"));
$sql .= "INSERT INTO osmchange (date,postnr,data) VALUES ('$dato','$postnummer','$sqltextexport');";
...
//echo "<script>parent.self.location='osmdata-change.php?postnummer=" . $nextpostnr  . "';</script>";
